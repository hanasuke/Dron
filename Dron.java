      ...
      while ( player1.getLiveStatus() && player2.getLiveStatus() ) {
        ...
        if (state[currentPoint1.y][currentPoint1.x]!=Color.WHITE) {
          ...
          if ( countMove > barSize ) {
            ...
            bMoveP1 = barP1.barRepaint(currentPoint1.y, currentPoint1.x);
            state[bMoveP1.y][bMoveP1.x] = Color.WHITE;
          } else {
            // enqueueのみ          
            barP1.queue.enqueue(currentPoint1.y, currentPoint1.x);
          }
        }
        player2.move();
        ...
        if (state[currentPoint2.y][currentPoint2.x]!=Color.WHITE) {
          ...
          if (countMove > barSize ) {
            // dequeue enqueue paint
            bMoveP2 = barP2.barRepaint(currentPoint2.y, currentPoint2.x);
            state[bMoveP2.y][bMoveP2.x] = Color.WHITE;
          } else {
            // enqueueのみ          
            barP2.queue.enqueue(currentPoint2.y, currentPoint2.x);
          }
        }
        countMove++;
